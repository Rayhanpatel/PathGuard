...
from pathguard.config import DEFAULT_VIDEO_PATH, CorridorParams, RuntimeParams
...
with st.sidebar:
    ...
    dino_prompt_path = st.text_input(
        "DINO prompt file (from Cactus Narrator)",
        value="",
        help="Path to a *_dino_prompt.txt file generated by the Cactus Narrator. "
             "Leave blank to use the default static prompt list.",
    )
...
if start:
    # Load dynamic prompts from Cactus if a file path was provided
    from pathguard.config import load_dynamic_prompts
    dynamic_prompts = load_dynamic_prompts(dino_prompt_path if dino_prompt_path.strip() else None)
